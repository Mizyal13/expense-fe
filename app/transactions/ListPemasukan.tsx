...
export default function ListPemasukan({
  ...
  // Debugging logs
  console.log("📥 ListPemasukan received transactions:", transactions);
  console.log("📥 ListPemasukan received categories:", categories);

  // Filter transaksi pemasukan
  const pemasukan = transactions.filter(
    (tx) => tx.tipe === "pemasukan" || (tx.tipe === null && tx.jumlah >= 0)
  );

  console.log("📊 Filtered pemasukan:", pemasukan);

  if (pemasukan.length === 0) {
    return <p className="text-gray-500">Belum ada pemasukan.</p>;
  }
